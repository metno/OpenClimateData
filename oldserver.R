...
server <- function(input, output, session) {
  ...
  observeEvent(input$season, {
    ...
    observe({
      ...
      if (is.null(selectedStid)) selectedStid <- getstid()
      ...
      output$datainterval <- renderText({
        paste(sources[is.element(src,input$src),as.numeric(input$lingo)],attr(Y,'period')[1],' - ',attr(Y,'period')[2])})
    })
  }